    ...
    def forward(self, x: torch.Tensor, ex_feats: Optional[torch.Tensor] = None, beta=1.2, gamma=3.0,
        ...
        if x.shape[1] != self.positional_embedding.shape[0]:
            x = x + self.interpolate_pos_encoding(
                x, input_height, input_width).to(x.dtype)
        else:
        ...
        token_size = grid_height, grid_width
        ...
        if return_cls:
            # CLS token from the standard last-layer forward (dropped by the proxy branch).
            standard = self.transformer.resblocks[-1](x)
            cls_feature = self.ln_post(standard[0]) @ self.proj
        for blk in self.transformer.resblocks[-1:]:
